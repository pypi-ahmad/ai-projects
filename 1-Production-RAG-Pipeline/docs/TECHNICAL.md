...
| `bm25s` | Separate, persisted lexical (BM25) index over chunk texts — **superseded the Phase 0 plan** of Qdrant native sparse vectors in the same collection, per an explicit Phase 4 instruction to persist a standalone BM25 index. Picked over `rank_bm25` (the other name that instruction allowed): `rank_bm25` hasn't shipped a release since Feb 2022 and has no built-in save/load; `bm25s` is actively maintained, pure Python + numpy (Windows-friendly, no Java/compiled extension), ships `save()`/`load()`, and is dramatically faster on its own published benchmarks. |
...
| `ruff` + `ty` + `pytest` | Lint/format, type-check, test — one fast tool per concern, no separate isort/flake8/black/mypy chain. |
...
Qdrant local mode takes an OS-level file lock while open (`.lock` above) — see
`docs/RUNBOOK.md`'s "locked index" failure mode. Neither store is meant to be human-readable or
...
Write tool cannot create `.env`-pattern files in this project (a global permission deny rule) —
the batch script doing this when a user later executes it is a different actor than the Write
