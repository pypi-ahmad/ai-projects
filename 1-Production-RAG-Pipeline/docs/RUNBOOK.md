...
Operational notes for running this pipeline on Windows 11.
...
**Note on `run.cmd` using plain `pip`/`venv`, not `uv`**: every other command in this runbook uses
`uv run ...` against the `uv`-managed dev environment (`pyproject.toml` + `uv.lock`, canonical).
`run.cmd` is a separate, self-contained path for someone who doesn't have `uv` installed — it
creates its own `.venv` via stdlib `venv` and installs from `requirements.txt`, which is
autogenerated from `uv.lock` (`uv export --no-hashes --no-dev --format requirements-txt`) and
must be regenerated after any `uv add`/`uv remove`, or `run.cmd`'s installs will drift from what
the dev environment actually uses.
...
Pull only what you need up front — `granite4.1:3b`, `qwen3-embedding:0.6b`, and `qwen3.5:0.8b`
cover the default path (generate, embed, rerank). The others are fallbacks or OCR/translation,
used only when their stage is triggered.
...
A provider only appears as available if all of its required variables are set — see
`Settings.available_providers()` in `src/rag_pipeline/config.py`. Ollama needs no key and is
always listed, but still requires the Ollama service itself to be running.
