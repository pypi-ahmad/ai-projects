...
Implemented in `src/rag_pipeline/eval/` (`metrics.py`, `judge.py`, `pipeline.py`, `cli.py`), plus
the Eval tab in the Streamlit UI. This replaces an earlier draft of this doc that described a
`queries.jsonl` format and a recall@5-only plan that was never built that way.
...
This is a real number from a real run, not a target — the sample corpus is tiny and each
question maps to exactly one obviously-relevant document, so a perfect score here says the
plumbing works end to end, not that retrieval quality is validated at any real scale.
