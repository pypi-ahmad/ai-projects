...
The pipeline has two paths: **ingest**, which builds the index, and **request**, which answers a
query with citations. Both are fully implemented in Phases 2 through 6 of `SPEC.md`. The
Streamlit UI and evaluation layer on top of those paths.
...
- Point identity is deterministic (`uuid5` of `doc_id:chunk_index`), so re-indexing a changed
  chunk updates it in place instead of creating a duplicate. See `docs/TECHNICAL.md`.
...
- Reranking is pointwise and uses `qwen3.5:0.8b`; the allowlist has no dedicated reranker model.
