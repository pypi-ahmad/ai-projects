...
- **One heavy Ollama model resident at a time** — unload before loading the next stage's model.
...
There is no dedicated reranker on the allowlist: rerank by prompting `qwen3.5:0.8b` with a tight
scoring prompt, batched, narrowing top 20 → top 5.

### OCR integration note (important, non-obvious)

`AuditAid/PaddleOCR-VL-1.6-0.9B` is a real, pullable Ollama model — it lives in Ollama's
**community namespace** (publisher `AuditAid`), not the official library, and it is **not** on
Hugging Face under that org (the official model is `PaddlePaddle/PaddleOCR-VL-1.6`; `AuditAid`
only republished a GGUF of it to Ollama's registry). It is only the VLM-recognition half of
PaddleOCR-VL — full document parsing (layout analysis + recognition) needs the `paddleocr` Python
package, which orchestrates layout detection itself and calls out to this model **through
...
So the heavy model stays 100% Ollama-resident (respecting the VRAM/one-model-at-a-time rule);
`paddleocr` (plus its `paddlepaddle` dependency for the lightweight layout-detection step) is a
coordination-layer dependency, not a second GPU-resident model. Verify `paddlepaddle` wheel
availability for Python 3.13 on Windows when building the OCR phase — it and `onnxruntime` (used
by FastEmbed) are the two dependencies most likely to lag a brand-new Python release.
...
Each phase writes/updates only the files it owns and runs whatever it can without the full
1000-doc corpus before handing off.
