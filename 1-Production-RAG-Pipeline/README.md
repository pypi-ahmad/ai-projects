...
**Status: all 7 phases done** — ingest, chunk, index, retrieve, generate, a Streamlit UI, eval,
and a double-clickable launcher. See "Limitations" below and `SPEC.md` for what's still thin.
...
If you'd rather not use the UI, the same steps as a one-liner CLI sequence:
...
There is no dedicated reranker on the allowlist: reranking is done by prompting
`qwen3.5:0.8b` with a scoring prompt, batched, narrowing top 20 candidates to top 5.
...
Double-click `run.cmd` — see `docs/RUNBOOK.md`'s click-path for what it does step by step (it
creates its own plain `venv` + `pip install`, separate from this repo's `uv`-managed dev
environment; both work, `run.cmd` just doesn't require `uv` to be installed). For development,
use `uv run <command>` directly instead (see `docs/RUNBOOK.md`).
...
- **8GB VRAM out-of-memory / everything falls back to CPU and is very slow.** Check no other
  ...
  they're opt-in, not a default upgrade.
...
- `src/rag_pipeline/eval/metrics.py` now has offline unit tests (`tests/test_eval_metrics.py`).
  ...
  verified only by direct CLI runs and static syntax/type checks.
