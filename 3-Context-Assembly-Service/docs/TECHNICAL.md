...
- `cl100k_base` is a close approximation for Claude and GPT-4 models; counts run 5 to 10% high on average, which is conservative (safer than under-counting).
...
| `priority` | `int` | Higher = kept first (0 to 100 convention) |
...
|------------|----------------|
| System prompt | 100 |
| Tool schemas (active) | 80 to 90 |
| Recent memory turns | 70 to 80 |
| High-relevance doc chunks | 50 to 70 |
| Older memory / lower-relevance docs | 10 to 50 |
| Tool results (verbose) | 20 to 50 |
