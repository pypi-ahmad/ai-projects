...
**Flow:** `preprocess → extract (gpt-6-sol + Pydantic Invoice) → Python math validate → (optional low-confidence crop/re-read → retry extract with error text, up to N times) → commit or human review (HITL)`. The model proposes fields; Python is the only thing that commits them; no silent number fixes.
...
**Process:** Work one phase at a time. Each phase implements its own scope,
updates its documentation, and stops. Report the created files and what the
