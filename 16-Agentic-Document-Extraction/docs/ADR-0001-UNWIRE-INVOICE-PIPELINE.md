...
correct — checking that `quantity * unit_price == amount` per line item,
...
medical forms (BadgeCare Plus, Amerigroup/RealSolutions-style intake forms —
...
relationship between any two fields — nothing for `src/validate.py`'s check
...
`parse` — layout parsing, Markdown/HTML rendering, and PDF/PNG annotation —
is the part of the original pipeline that doesn't depend on the invoice
arithmetic contract, so it stays active and is the entire current product:
a best-effort, un-validated layout reading of the document for a human to
...
- The active product no longer implies a correctness guarantee
  ...
  correction, not just a cosmetic one (see the rewrite of
