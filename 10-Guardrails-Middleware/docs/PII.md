...
| `phone` | `[PHONE]` | Generic pattern **and** an Indian mobile pattern (`+91`-optional, 10 digits starting 6–9). See false-positive risk below. |
| `credit_card` | `[CARD]` | 13–19 digit candidate, confirmed via Luhn check. |
...
| Type | Placeholder | Notes |
|---|---|---|
| `aadhaar_possible` | `[AADHAAR_POSSIBLE]` | 12-digit shape, first digit 2–9, optionally grouped 4-4-4. **Shape only — no checksum, no legal validation.** A match is not a confirmation of a real Aadhaar number. |
...
- **Phone.** Both patterns are shape-only. The generic pattern's optional country-code group
  means it can match *any* contiguous 10–13 digit run (order numbers, reference codes, even
  parts of a credit-card-shaped or Aadhaar-shaped number). The Indian mobile pattern matches any
  10-digit number starting 6–9, which is far broader than actual assigned mobile numbers.
