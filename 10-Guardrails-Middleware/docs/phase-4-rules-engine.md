...
denied-topics blocklists — all config-driven, severity included. Unlike Phases 2–3, this phase
...
- `config/blocklists/{injection,role_play,leak_phrases,denied_topics}.txt` — short, generic
  phrase lists (5–8 lines each; `denied_topics.txt` ships empty). One phrase per line
...
- **No large jailbreak dataset.** Each blocklist file is 4–7 generic phrases, matching the
...
Phases 2–3 (`test_pii.py`, `test_detectors.py`, `test_pipeline.py`).
