...
There is no single allowlist across all projects. Several children reuse this catalogue; others use
smaller registries or a configurable model name. The selected child's provider and configuration code
determine what each operation accepts. Pull only the models you need.
...
Build a document-to-answer pipeline that ingests mixed files, chunks them, combines dense and BM25
retrieval, reranks results, and generates cited answers. Start with `data/raw/sample/`, run ingestion and
indexing, then ask a question. Ollama is needed for local embeddings and reranking, even with cloud answer
generation. The documented local setup targets an 8 GB NVIDIA GPU. Cloud keys are optional. Inspect the
sources; citations alone do not prove correctness.
...
Build a token-budgeted packer for supplied memory, document, and tool blocks. It reports what it kept,
marked for compression, or dropped. Load the sample and export the packed messages as JSON. Packing needs
no GPU, Ollama, or API credentials. Optional model-based compression uses Ollama or a configured cloud
provider. The caller supplies the blocks.
...
Start: `.\run.cmd` (API only). For the optional UI, run `uv run streamlit run src/stream/ui.py` in a
